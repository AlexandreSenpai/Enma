...
class NHentaiAsync:
    ...
    async def get_doujin(self, doujin_id: int):
        return await GetDoujinUseCase(nhentai_repo=self._NHENTAI_ADAPTER).execute(doujin_id=doujin_id)

    async def get_random(self) -> Doujin:
        return await GetRandomDoujinUseCase(nhentai_repo=self._NHENTAI_ADAPTER).execute()
    
    async def search(self, query: str, page: int = 1, sort: Sort = Sort.RECENT):
        return await SearchDoujinUseCase(nhentai_repo=self._NHENTAI_ADAPTER).execute(query=query, page=page, sort=sort)
    
    async def get_popular_now(self) -> PopularPage:
        return await GetPopularNowUseCase(nhentai_repo=self._NHENTAI_ADAPTER).execute()