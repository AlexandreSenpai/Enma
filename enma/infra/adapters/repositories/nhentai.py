...
from enma.domain.entities.manga import (
    MIME,
    Chapter,
    Genre,
    Author,
...
from enma.infra.core.interfaces.nhentai_response import (
    NHentaiImage,
    NHentaiResponse,
    Tag as NHentaiResponseTag,
)
...
class CloudFlareConfig(BaseModel):
    ...
    def user_agent_validator(self, value: str) -> str:
        if value == "":
            raise InvalidRequest(message="User Agent cant be empty.")
        return str(value)

    @field_validator("cf_clearance")
    def cf_clearance_validator(self, value: str) -> str:
        if value == "":
...
class NhentaiSourceConfig(BaseModel):
    ...
    image_base_url: str = Field(
        default="https://{server}.nhentai.net/galleries/"
    )
    tiny_image_url: str = Field(
        default="https://{server}.nhentai.net/galleries/"
    )
    avatar_url: str = Field(default="https://i5.nhentai.net/")

    def model_post_init(self, ctx) -> None:
        self.image_base_url = self.image_base_url.format(
            server=self.large_image_server
        )
        self.tiny_image_url = self.tiny_image_url.format(
            server=self.tiny_image_server
        )
...
class NHentai(IMangaRepository):
    ...
    def __init__(self, config: Optional[NHentaiConfig] = None) -> None:
    ...
    def set_config(self, config: NHentaiConfig) -> None:
        if not isinstance(config, NHentaiConfig):
            raise InvalidConfig(
                message="You must provide a NHentaiConfig object."
            )
        config = config or NHentaiConfig()
    ...
    def __handle_source_response(self, response: requests.Response):
        logger.debug(
            f"Fetched {response.url} with response status code {response.status_code} and text {response.text}"
        )
        if response.status_code == 200:
            return
        if response.status_code == 403:
            raise Forbidden(
                message="Could not perform a successfull request to the source due credentials issues. \
Check your credentials and try again."
            )
        if response.status_code == 404:
            raise NotFound(
                message=f'Could not find the requested resource at "{response.url}". \
Check the provided request parameters and try again.'
            )
        raise Unknown(
            message="Something unexpected happened while trying to fetch source content. \
Set the logging mode to debug and try again."
    ...
    def __make_request(
        ...
        user_agent = (
            config.user_agent if config is not None else f"Enma/{__version__}"
        )
        cookies = (
            {"cf_clearance": config.cf_clearance} if config is not None else {}
        )

        logger.debug(
            f"Fetching {url} with headers {headers}, params {params} and cookies: {cookies}"
        )
    ...
    def __make_page_uri(
        ...
        if type == "cover":
            url = urljoin(
                self.__TINY_IMAGE_BASE_URL, f"{media_id}/cover.{mime.value}"
            )
        elif type == "thumbnail":
            url = urljoin(
                self.__TINY_IMAGE_BASE_URL, f"{media_id}/thumb.{mime.value}"
            )
        else:
            url = urljoin(
                self.__IMAGE_BASE_URL, f"{media_id}/{page_number}.{mime.value}"
            )
    ...
    @Cache(
        max_age_seconds=int(
            os.getenv("ENMA_CACHING_FETCH_SYMBOLIC_LINK_TTL_IN_SECONDS", 100)
        ),
        max_size=20,
    ).cache
        ...
        if with_symbolic_links:
            ...
            for index, page in enumerate(pages):
                ...
                if safe_mime is None:
                    logger.warning(
                        f"Could not find a valid mime type for page {index + 1}. Forcing mime type as JPG."
                    )
    ...
    def __get_tag_by_type(
        self, type: TagType, tags: List[NHentaiResponseTag]
    ) -> List[NHentaiResponseTag]:
        return list(filter(lambda tag: tag.get("type") == type, tags))

    @Cache(
        max_age_seconds=int(os.getenv("ENMA_CACHING_GET_TTL_IN_SECONDS", 300)),
        max_size=20,
    ).cache
    def get(
        self, identifier: str, with_symbolic_links: bool = False, **kwargs
    ) -> Union[Manga, None]:
        ...
        language = [
            tag.get("name")
            for tag in self.__get_tag_by_type(
                type="language", tags=nhentai_tags
            )
        ]

        authors = [
            Author(id=tag.get("id"), name=tag.get("name"))
            for tag in self.__get_tag_by_type(type="artist", tags=nhentai_tags)
        ]

        genres = [
            Genre(id=tag.get("id"), name=tag.get("name"))
            for tag in self.__get_tag_by_type(type="tag", tags=nhentai_tags)
        ]

        characters = [
            EnmaTag(type="character", name=tag.get("name"), id=tag.get("id"))
            for tag in self.__get_tag_by_type(
                type="character", tags=nhentai_tags
            )
        ]

        related = [
            EnmaTag(type="related", name=tag.get("name"), id=tag.get("id"))
            for tag in self.__get_tag_by_type(type="parody", tags=nhentai_tags)
        ]

        category = [
            EnmaTag(type="category", name=tag.get("name"), id=tag.get("id"))
            for tag in self.__get_tag_by_type(
                type="category", tags=nhentai_tags
            )
        ]

        tags = [*characters, *related, *category]

        safe_mime = mime.get_mime_safelly(
            doujin.get("images").get("thumbnail").get("t").upper()
        )
        safe_mime = safe_mime if safe_mime is not None else MIME.J

        thumbnail = Image(
            uri=self.__make_page_uri(
                type="thumbnail", mime=safe_mime, media_id=media_id
            ),
            mime=safe_mime,
        ...
        safe_mime = mime.get_mime_safelly(
            doujin.get("images").get("cover").get("t").upper()
        )
        safe_mime = safe_mime if safe_mime is not None else MIME.J

        cover = Image(
            uri=self.__make_page_uri(
                type="cover", media_id=media_id, mime=safe_mime
            ),
            mime=safe_mime,
    ...
    @Cache(
        max_age_seconds=int(
            os.getenv("ENMA_CACHING_SEARCH_TTL_IN_SECONDS", 100)
        ),
        max_size=5,
    ).cache
    def search(
        self, query: str, page: int, sort: Sort = Sort.RECENT
    ) -> SearchResult:
        logger.debug(
            f"Searching into Nhentai with args query={query};page={page};sort={sort}"
        )
        ...
        search_result = SearchResult(
            query=query, total_pages=0, page=page, total_results=0, results=[]
        )
        ...
        logger.debug(
            f"Found successfully search result container using .class.container."
        )
        pagination_container = soup.find("section", {"class": "pagination"})
        logger.debug(
            f"Found successfully pagination container using .class.pagination."
        )

        last_page_a_tag = (
            pagination_container.find("a", {"class": "last"})
            if pagination_container
            else None
        )  # type: ignore
        logger.debug(f"Found last pagination container using .calss.last.")
        total_pages = (
            int(last_page_a_tag["href"].split("=")[-1])
            if last_page_a_tag
            else 1
        )  # type: ignore
        logger.debug(
            f"Found last page number successfully splitting last pagination number href."
        )

        if not search_results_container:
            return search_result

        search_results = search_results_container.find_all(
            "div", {"class": "gallery"}
        )  # type: ignore
        ...
        search_result.total_results = (
            25 * total_pages if pagination_container else len(thumbs)
        )
    ...
    @Cache(
        max_age_seconds=int(
            os.getenv("ENMA_CACHING_PAGINATE_TTL_IN_SECONDS", 100)
        ),
        max_size=5,
    ).cache
    ...
    def random(self, retry=0) -> Manga:
        ...
        logger.debug(
            f'Got successfully random manga id {id} from "gallery_id.h3" tag.'
        )
        ...
        if doujin is None:
            if retry >= 4:
                raise ExceedRetryCount(
                    "Could not fetch a random doujin after 4 retries."
                )
    ...
    @Cache(
        max_age_seconds=int(
            os.getenv("ENMA_CACHING_AUTHOR_TTL_IN_SECONDS", 100)
        ),
        max_size=5,
    ).cache
    def author_page(self, author: str, page: int) -> AuthorPage:
        request_response = self.__make_request(
            url=urljoin(self.__BASE_URL, f"artist/{author}"),
            params={"page": page},
        )

        result = AuthorPage(
            author=author, total_pages=0, page=page, total_results=0, results=[]
        )
