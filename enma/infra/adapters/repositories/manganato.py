...
class Manganato(IMangaRepository):
    ...
    def __create_title(self, main_title: str, alternative: str) -> Title:
        logger.debug(
            f"Building manga title main: {main_title} and alternative: {alternative}"
        )

        has_many_alternatives = (
            alternative.find(";") != -1 or alternative.find(",") != -1
        )

        if not has_many_alternatives:
            jp = alternative
            return Title(
                english=main_title.strip(),
                japanese=jp.strip(),
        ...
        jp, cn, *_ = (
            ...
            else alternative.split(",")
        )
        return Title(
            english=main_title.strip(), japanese=jp.strip(), other=cn.strip()
        )

    def __find_chapters_list(self, html: BeautifulSoup) -> list[str]:
        chapter_list = cast(
            Tag, html.find("ul", {"class": "row-content-chapter"})
        )
    ...
    def __create_chapter(
        self, url: str, symbolic: bool = False
    ) -> Union[Chapter, None]:
        if symbolic:
        ...
        if response.status_code != 200:
            logger.error(
                f"Could not fetch the chapter with url: {url}. status code: {response.status_code}"
            )
            return

        chapter = Chapter(id=response.url.split("/")[-1])
        html = BeautifulSoup(response.text, "html.parser")
        images_container = cast(
            Tag, html.find("div", {"class": "container-chapter-reader"})
        )
    ...
    def get(
        self, identifier: str, with_symbolic_links: bool = False
    ) -> Union[Manga, None]:
        response = self.__make_request(
            url=urljoin(self.__CHAPTER_BASE_URL, identifier)
        )

        if response.status_code != 200:
            logger.error(
                f"Could not fetch the manga with identifier: {identifier}. status code: {response.status_code}"
            )
            return
        ...
        title = self.__create_title(
            main_title=elem_title, alternative=title_cel.text
        )
        ...
        extra_infos = cast(
            Tag, soup.find("div", {"class": "story-info-right-extent"})
        )

        updated_at = None
        if extra_infos:
            updated_at_field = extra_infos.find_all("p")[0]
            updated_at = updated_at_field.find(
        ...
        if with_symbolic_links:
            ...
            chapters = [
                self.__create_chapter(link, symbolic=True)
                for link in chapters_links
            ]
        else:
            workers = cpu_count()
            logger.debug(
                f"Initializing {workers} workers to fetch chapters of {identifier}."
            )

            with ThreadPoolExecutor(max_workers=workers) as executor:
                chapters = executor.map(
                    self.__create_chapter, self.__find_chapters_list(html=soup)
                )
                chapters = list(
                    filter(lambda x: isinstance(x, Chapter), list(chapters))
                )
                executor.shutdown()
        ...
        return Manga(
            ...
            id=identifier,
            created_at=datetime.strptime(updated_at, "%b %d,%Y - %H:%M %p")
            if updated_at
            else None,
            updated_at=datetime.strptime(updated_at, "%b %d,%Y - %H:%M %p")
            if updated_at
            else None,
    ...
    def search(self, query: str, page: int) -> SearchResult:
        response = self.__make_request(
            url=urljoin(
                f"{self.__BASE_URL}/search/story/", "_".join(query.split())
            ),
            params={"page": page},
        )
        ...
        for result in results:
            thumbs.append(
                Thumb(
                    title=result.find("h3").text.replace("\n", "").strip(),
                    url=result.find(
                        "a", {"class": "a-h text-nowrap item-title"}
                    )["href"],
                    cover=Image(
                        uri=result.find("img")["src"], width=0, height=0
                    ),
                    id=result.find(
                        "a", {"class": "a-h text-nowrap item-title"}
                    )["href"].split("/")[-1],
        ...
        return SearchResult(
            query=query, page=page, total_pages=total_pages, results=thumbs
        )
    ...
    def paginate(self, page: int) -> Pagination:
        response = self.__make_request(
            url=f"{self.__BASE_URL}/genre-all/{page}"
        )
        ...
        content_items = content_panel.find_all(
        ...
        for item in content_items:
            item = cast(Tag, item)
            info = cast(
                Tag, item.find("a", {"class": "genres-item-img bookmark_check"})
            )
            cover = info.find("img")
            pagination.results.append(
                Thumb(
                    id=info["href"].split("/")[-1],  # type: ignore
                    url=info["href"],  # type: ignore
                    title=info["title"] if info is not None else "",  # type: ignore
                    cover=Image(uri=cover["src"], width=0, height=0),
                )
            )  # type: ignore
