...
from enma.infra.core.interfaces.mangadex_response import (
    AuthorRelation,
    CoverArtRelation,
    IAltTitles,
    IGetResult,
...
class Mangadex(IMangaRepository):
    ...
    def __init__(self) -> None:
        ...
        self.__CHAPTER_PAGE_URL = (
            "https://cmdxd98sb0x3yprd.mangadex.network/data/"
        )
    ...
    def __handle_source_response(self, response: requests.Response):
        ...
        to indicate various error conditions such as rate limits exceeded,
        forbidden access, or resource not found.

        Args:
            response (Response): The HTTP response object from a request to
            the Mangadex API.
        ...
        logger.debug(
            f"Fetched {response.url} with response status code "
        ...
        if response.status_code == 403:
            raise Forbidden(
                message="Could not perform a successful request to the source "
                "due to credentials issues. Check your credentials and try again."
            )
        if response.status_code == 404:
            raise NotFound(
                message="Could not find the requested resource at "
                '"{response.url}". Check the provided request parameters and '
                "try again."
            )
        if response.status_code == 429:
            raise ExceedRateLimit(
                message="You have exceeded the Mangadex rate limit!"
            )
        raise Unknown(
            message="Something unexpected happened while trying to "
            "fetch source content. Set the logging mode to debug and try again."
        )
    ...
    def __make_request(
        ...
        params: Optional[
            Union[dict[str, Union[str, int]], list[tuple[str, Union[str, int]]]]
        ] = None,
    ) -> requests.Response:
        """
        Makes a request to the specified URL with the given
        headers and parameters.

        Args:
            url (str): The URL to make the request to.
            headers (dict[str, Any], optional): The headers to include in the
        ...
        logger.debug(
            f"Fetching {url} with headers {headers} and params {params}"
        )
    ...
    def __create_cover_uri(self, manga_id: str, file_name: str) -> str:
        """
    ...
    def fetch_chapter_by_symbolic_link(self, link: SymbolicLink) -> Chapter:
        ...
        for index, page in enumerate(ch.get("chapter").get("data")):
            ...
            if safe_mime is None:
                logger.warning(
                    f"Could not determine MIME type for extension {extension}. Defaulting to JPEG."
                )
            ...
            chapter.add_page(
                Image(
                    uri=self.__create_chapter_page_uri(
                        ch.get("chapter").get("hash"), page
                    ),
    ...
    def __create_chapter(
        self, chapter: tuple[int, str], with_symbolic_links: bool = False
    ) -> Chapter:
        """
        ...
        if with_symbolic_links:
            ...
            for index, page in enumerate(files):
                ...
                ch.add_page(
                    Image(
                        uri=self.__create_chapter_page_uri(hash, page),
                        name=f"{index}.{extension}",
                        mime=safe_mime,
                    )
                )
    ...
    def __list_chapters(self, manga_id: str) -> list[tuple[int, str]]:
        ...
        response = self.__make_request(
            url=urljoin(self.__API_URL, f"manga/{manga_id}/aggregate")
        )
        ...
        for volume in data.get("volumes", dict()):
            ...
            for volume_key in volume_chapters:
                ...
                chapters.append(
                    (current_vol.get("chapter"), current_vol.get("id"))
                )
    ...
    def __extract_authors(self, relations: IRelations) -> list[Author]:
        ...
        if len(authors_data) > 0:
            for author in authors_data:
                ...
                authors.append(
                    Author(
                        name=author.get("attributes").get("name"),
                        id=author.get("id"),
                    )
                )
    ...
    def __get_cover(self, manga_id: str, relations: IRelations) -> Image:
        """
        ...
        return Image(
            uri=self.__create_cover_uri(
                manga_id, cover.get("attributes").get("fileName")
            ),
            width=512,
        )
    ...
    def __get_title(self, alt_titles: IAltTitles, title: str) -> Title:
        ...
        japanese_titles = [
            ...
            if title.get("ja-ro") is not None
        ]
        japanese_title = (
            japanese_titles[0] if len(japanese_titles) > 0 else None
        )
        ...
        return Title(
            ...
            other=alt_titles[-1].get(other_key, "")
            if len(alt_titles) > 0
            else "",
        )

    def __parse_full_manga(
        self, manga_data: IManga, with_symbolic_links: bool = False
    ) -> Manga:
        """
        ...
        thumbnail = self.__get_cover(
            manga_data.get("id"), manga_data.get("relationships")
        )
        ...
        manga = Manga(
            ...
            language=Language.get(
                attrs.get("originalLanguage").strip().lower().replace("-", "_"),
                "unknown",
            ),
            authors=self.__extract_authors(
                manga_data.get("relationships", list())
            ),
        ...
        for chapter in chapter_list:
            manga.add_chapter(
                self.__create_chapter(
                    chapter=chapter, with_symbolic_links=with_symbolic_links
                )
            )
    ...
    def get(self, identifier: str, with_symbolic_links: bool = False) -> Manga:
        ...
        """
        response = self.__make_request(
            url=urljoin(self.__API_URL, f"manga/{identifier}"),
            params=[
                ("includes[]", "cover_art"),
                ("includes[]", "author"),
                ("includes[]", "artist"),
        ...
        return self.__parse_full_manga(
            manga_data=manga_data, with_symbolic_links=with_symbolic_links
        )
    ...
    def __make_sort_query(self, sort: Sort) -> dict[str, str]:
        ...
        """
        return {
            f"order[{sort.value if isinstance(sort, Sort) else sort}]": "desc"
        }
    ...
    def search(
        ...
        """
        logger.debug(
            f"Searching into Mangadex with args query={query};page={page};sort={sort}"
        )

        params = [
            ("title", query),
            *tuple(self.__make_sort_query(sort).items()),
            ("includes[]", "cover_art"),
            ("limit", per_page),
        ...
        request_response = self.__make_request(
            url=urljoin(self.__API_URL, "manga"), params=params
        )
    ...
    def paginate(self, page: int) -> Pagination:
        ...
        per_page = 25
        request_response = self.__make_request(
            url=urljoin(self.__API_URL, "manga"),
            params=[
                ("limit", per_page),
                ("offset", per_page * (page - 1) if page > 1 else 0),
                ("order[createdAt]", "desc"),
    ...
    def random(self, retry: int = 0) -> Manga:
        """
        ...
        """
        response = self.__make_request(
            url=urljoin(self.__API_URL, "manga/random"),
            params=[
                ("includes[]", "cover_art"),
                ("contentRating[]", "safe"),
                ("contentRating[]", "suggestive"),
        ...
        return self.__parse_full_manga(
            manga_data=manga, with_symbolic_links=True
        )
