...
class GoogleDriveStorage(ISaverAdapter):
    def __init__(self, credentials_path: str, root_shared_folder: str):
        try:
    ...
    def save(self, path: str, file: File) -> bool:
        try:
            ...
            logger.debug(
                f"Uploading image to google drive with name: {file.name} and parent folder: {folder_id}"
            )

            media = self.MediaIoBaseUpload(
                file.data, mimetype="application/octet-stream"
            )
            ...
            logger.error(
                f"A HTTP error ocurred while trying to upload image to google drive: {e}"
            )
            return False
        except Exception as e:
            logger.error(
                f"An unknown error ocurred while trying to upload image to google drive: {e}"
            )
            return False
    ...
    def _get_or_create_folder(self, path: str) -> str:
        ...
        for folder_name in folder_names:
            ...
            if items:
                ...
                logger.debug(
                    f"Creating folder with metadata: {folder_metadata}"
                )
