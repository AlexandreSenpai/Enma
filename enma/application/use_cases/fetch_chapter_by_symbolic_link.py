...
class FetchChapterBySymbolicLinkUseCase(
    IUseCase[
    ...
    def execute(
        self, dto: DTO[FetchChapterBySymbolicLinkRequestDTO]
    ) -> FetchChapterBySymbolicLinkResponseDTO:
        logger.info(f"Fetching chapter by symbolic link: {dto.data.link}.")

        chapter = self.__manga_repository.fetch_chapter_by_symbolic_link(
            link=dto.data.link
        )
