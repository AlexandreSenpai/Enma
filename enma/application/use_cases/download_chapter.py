...
class DownloadChapterRequestDTO:
    ...
    threaded: Optional[Threaded] = field(
        default_factory=lambda: Threaded(use_threads=False, number_of_threads=0)
    )
...
class DownloadChapterUseCase(
    ...
    def __init__(self) -> None:
        self.downloader: Union[IDownloaderAdapter, None] = None
        self.queue: Union[queue.Queue[File], None] = None

    def __make_download_with_queue(self, page: Image) -> None:
        if self.downloader is None:
            raise ValueError("Downloader is not set.")
    ...
    def __handle_saving(
        self, save_queue: queue.Queue[File], path: str, saver: ISaverAdapter
    ) -> None:
        while True:
            try:
                content = save_queue.get(timeout=5)
                logger.debug(
                    f"Saving file to path: {path} with name: {content.name}"
                )
    ...
    def __spawn_workers(self, chapter: Chapter, workers: int) -> None:
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(self.__make_download_with_queue, page): page
    ...
    def __make_sync_download(
        self, chapter: Chapter, downloader: IDownloaderAdapter
    ) -> None:
        for page in chapter.pages:
    ...
    def execute(
        self, dto: DTO[DownloadChapterRequestDTO]
    ) -> DownloadChapterResponseDTO:
        try:
            using_threads = dto.data.threaded is not None

            logger.info(
                f"Downloading chapter with {len(dto.data.chapter.pages)} pages."
            )
            ...
            logger.debug(
                f"Spawning {queue_threads_num} threads to handle download queue."
            )
            for _ in range(queue_threads_num):
                t = threading.Thread(
                    target=self.__handle_saving,
                    args=(self.queue, dto.data.path, dto.data.saver_adapter),
                )
                t.start()
                threads.append(t)
            ...
            if dto.data.threaded is not None:
                self.__spawn_workers(
                    chapter=dto.data.chapter,
                    workers=dto.data.threaded.number_of_threads,
                )
            else:
                self.__make_sync_download(
                    chapter=dto.data.chapter, downloader=dto.data.downloader
                )
